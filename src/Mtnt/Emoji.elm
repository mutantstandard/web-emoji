module Mtnt.Emoji exposing ( Emoji
                     , MM(..)
                     , Morph(..)
                     , CM(..)
                     , Color(..)
...
'src' is ignored in decoding because 'src' is the file used in 
...
isValidModifierPairing mm cm =
    case cm of
        CMUnsupported ->
            case mm of
                MMUnsupported -> True -- It's fine if they're both none.
...
{-| Checks an emoji to see if it's morph-modifiable.
-}
isMorphModifiable : Emoji -> Bool
isMorphModifiable emoji =
    emoji.morph /= MMUnsupported


{-| Checks an emoji to see if it's color-modifiable.
-}
isColorModifiable : Emoji -> Bool
isColorModifiable emoji =
    emoji.color /= CMUnsupported


{-| Checks an emoji to see if it's color OR morph modifiable.
-}
isAnyModifiable : Emoji -> Bool
isAnyModifiable emoji =
    emoji.morph /= MMUnsupported || emoji.color /= CMUnsupported
...
{-| Checks if an emoji has codepoint(s).
-}
hasCode : Emoji -> Bool
hasCode emoji = List.isEmpty emoji.code


{-| Checks if an emoji is a Unicode Standard emoji.
-}
isUnicode : Emoji -> Bool
isUnicode emoji =
    not (hasCode emoji) && not (inMtntPUA emoji)


{-| Checks if two emoji have the same root.
-}
sameRoot : Emoji -> Emoji -> Bool
sameRoot a b =
    a.root == b.root
